<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <link rel="stylesheet" href="../src/tuitui-ui.css">
  <title>按钮的设计与开发</title>
</head>
<body>
  <div class="tt-content">
    <!-- 标准按钮 -->
    <h2 class="tt-panel-title">标准按钮</h2>
    <div class="tt-panel-body">
      <a class="tt-btn">标准按钮</a>
      <br>
      <a class="tt-btn tt-btn-primary">主操作按钮</a>
      <br>
      <a class="tt-btn tt-btn-warning">警示按钮</a>
      <br>
      <a class="tt-btn tt-btn-disabled">不可用按钮</a>
      <br>
    </div>

    <!-- 大按钮 -->
    <h2 class="tt-panel-title">大按钮</h2>
    <div class="tt-panel-body no-padding">
      <a class="tt-btn tt-btn-large">大按钮</a>
      <br>
      <a class="tt-btn tt-btn-primary tt-btn-large">主操作按钮</a>
      <br>
      <a class="tt-btn tt-btn-warning tt-btn-large">警示按钮</a>
      <br>
      <a class="tt-btn tt-btn-disabled tt-btn-large">不可用按钮</a>
      <br>
    </div>

    <!-- 小按钮 -->
    <h2 class="tt-panel-title">小按钮</h2>
    <div class="tt-panel-body">
      <a class="tt-btn tt-btn-small">小按钮</a>
      <a class="tt-btn tt-btn-primary tt-btn-small">主操作按钮</a>
      <a class="tt-btn tt-btn-warning tt-btn-small">警示按钮</a>
      <a class="tt-btn tt-btn-disabled tt-btn-small">不可用按钮</a>
    </div>
  </div>
</body>
</html>